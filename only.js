...
setInterval(async () => {
	await saveUsers();
	console.log('СКРИПТ ОТ КАЛАНА ТОЛЬКО ВВЕРХ (Godzila)');
}, 30000);
...
updates.on('message', async (message) => {
	...
	{
		message.send (`Я вижу ты новенький! Рад познакомиться, отправь «помощь» что бы узнать мои команды. 🎲
...
[
{ "action": { "type": "text", "payload": "{\"button\": \"1\"}", "label": "🎮 Игры" }, "color": "negative" }, 
{ "action": { "type": "text", "payload": "{}", "label": "📚 Помощь" }, "color": "negative" }, 
{ "action": { "type": "text", "payload": "{}", "label": "🔐 Взломать" }, "color": "positive" }
], 
[
...
cmd.on(/^(?:гулять|🍀 Гулять)$/i, async (message, bot) => {
	let user = message.user
     let id = message.user.id
...
cmd.on(/^(?:помощь|📚 Помощь|команды|меню|help|commands|cmds|начать|start|хелп|Помощь|в меню|⬅ Назад)$/i, async (message, bot) => {
	message.sendSticker(50459)
	message.user.foolder += 1;
	await bot(`Раздел игровых команд:
...
cmd.on(/^(?:кейс инфо 1)/i, async (message) => {
	...
	*в отличии от начинающего кейса, в платинум кейсе выпадают призы в 50-ти кратном размере.`)
	...
	cmd.on(/^(?:кейсы 2)\s(.*)$/i, async (message, bot) => {
	message.args[1] = message.args[1].replace(/(\.|\,)/ig, '');
	message.args[1] = message.args[1].replace(/(к|k)/ig, '000');
	message.args[1] = message.args[1].replace(/(м|m)/ig, '000000');
...
cmd.on(/^(?:дайвинг|плавать|🎏 дайвинг|🎣 плавать|🎣 дайвинг|🐠 Дайвинг|🐠 Дайвинг!)$/i, async (message, bot) => { 
			
			if(message.user.timers.daiving > Date.now()) return bot(`Баллон с воздухом наполняется, подождите 5 минут 🎈`); 
		...
		cmd.on(/^(?:охота|🏹 Охота|охотиться|сходить поохотиться|🏹 Охотиться|🏹 Охотиться!|🏹 Охота!)$/i, async (message, bot) => {
			...
			message.user.timers.ohota = Date.now() + 300000;
...
cmd.on(/^(?:🎮 Игры|Игры)$/i, async (message, bot) => {
	message.user.foolder += 1;
	await bot(`🚀 Список игр:
...
cmd.on(/^(?:🍀 Основные|Основные)$/i, async (message, bot) => {
	message.user.foolder += 1;
	await bot(`📒 Профиль
     💲 Баланс
     👑 Рейтинг - ваш рейтинг
	 ...
	 🎁 Реферал - получайте бонусы за друзей!
...
cmd.on(/^(?:реф|реферал|🎁 Реферал)$/i, async (message, bot) => {
	message.user.foolder += 1;
	...
	🤑 За каждого друга вы получаете 500.000$
	Если друг активирует вашу рефералку, то он получит 100.000$`);
});
...
cmd.on(/^(?:реф|реферал)\s([0-9]+)$/i, async (message, bot) => {
	...
	[💲] >> За это, вам начислено 100.000$`);
});
cmd.on(/^(?:💰 Доход|Доход|Заработок)$/i, async (message, bot) => {
	message.user.foolder += 1;
	await bot(`👔 » Работы - список работ
     🔨 Работать
     ❌ Уволиться
...
cmd.on(/^(?:таксовать|🚕 Таксовать)$/i, async (message, bot) => { 
...
taxicash = utils.random(9872, 34160);
...
cmd.on(/^(?:👮 Взломать|взломать|взлом|🔐 Взломать)$/i, async (message, bot) => { 

if(message.user.timers.hack) return bot(`Что бы вас не расскрыли, вы сможете взломать только через 5 минут`);
...
🔹Что бы купить донат, перейдите по ссылке https://vk.com/godzila_bot
`)
});
...
🔹Что бы купить Валюту, перейдите по ссылке https://vk.com/godzila_bot
`)
});
/////
...
1.Беседа 
2.Беседа
 ☺😉напишите vk.com/godzila ему что вы хотите свою беседу добавить
...
cmd.on(/(?:профиль|проф|прф|аккаунт|Профиль|🔍 Профиль)$/i, async (message, bot) => {
...
cmd.on(/^(?:баланс|💸 Баланс)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
cmd.on(/^(?:Статистика)$/i, async (message, bot) =>{
	message.user.foolder += 1;
    return bot(`
	 🔧 Техническая информация бота @godzila_bot (Godzila) 🔧
⚙ » OC: Windows 10
🔐 » Версия бота: 2.5
...
cmd.on(/^(?:🦊 Питомец|Питомец)$/i, async (message, bot) => {
...
cmd.on(/^(?:фермы|@club195595178🔋 Ферма )\s?([0-9]+)?$/i, async (message, bot) => {
	message.user.foolder += 1;
